...
def make_fast_spline(scans, fast, group, bxid):
    ...
    for variable in fast.columns:
        ...
        group_list.append(group_val[["time","bxid", variable]])
    group_final = scans
    group_final = group_final.merge(group_list[0], how='left', on=["time"])
    for group_var in group_list[1:]:
        group_final = group_final.merge(group_var, how='left', on=["time","bxid"])
    return group_final


def make_dc(fill, overwrite=True):
    if not overwrite:
        dc_spline = pd.read_csv(f'Data/dc_spline.novdm.{fill}.gz', index_col=0).reset_index(drop=True)
        return dc_spline

    online = pd.read_csv(f'Data/lumivar_parse.{fill}.csv')
    ...
    #     print(dc_val)
        ...
        dc_val = dc_val[1:-1]
        data_dc = online.merge(dc_val, how='outer', on='time')
        ...
        dc_list.append(dc_spline[["time", variable]])
        
    dc_final = online
    print(dc_final)
        
    for dc_var in dc_list:
        dc_final = dc_final.merge(dc_var, how='left', on=["time"])
    ...
    dc_final.to_csv(f'Data/dc_spline.novdm.{fill}.gz', compression='gzip')
    dc_final.to_csv(f'Data/dc_spline.novdm.{fill}.csv')
...
def open_csv_fast(fill):
    ...
    # get data file names
    filenames = glob.glob(f"Data/fast.{fill}.*.csv")
...
def make_fast(fill, overwrite=True):
    if not overwrite:
        fast_spline = pd.read_csv(f'Data/fast_spline.novdm.{fill}.gz', index_col=0).reset_index(drop=True)
        return fast_spline
    online = pd.read_csv(f'Data/lumivar_parse.{fill}.csv')
    ...
    for bxid in fast.bxid.unique():
        ...
        fast_dic[bxid] = make_fast_spline(scans=online, fast=fast, group=fast_dic[bxid], bxid=bxid)
    ...
    fast_spline.to_csv(f'Data/fast_spline.novdm.{fill}.gz', compression='gzip')
    fast_spline.to_csv(f'Data/fast_spline.novdm.{fill}.csv')
