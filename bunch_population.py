...
#         # .toPandas()
    ...
    # data["nxcals_variable_name"] = data["nxcals_variable_name"].str.findall('(.*B[1-2])')
    ...
    data["nxcals_value"] = data['nxcals_value'].map(lambda x: x['elements'])
    data.to_csv(f"Data/fast.raw.{name}.csv")
...
#     data = pd.DataFrame.from_records(data.collect(), columns=data.columns)
    ...
    # data = data.nxcals_N.rename_axis([None], axis=1).reset_index()
    data.to_pandas().to_csv(f"Data/dc.raw.{name}.csv")
...
# time = ('2022-06-13 12:00:00.000', '2022-06-13 12:30:00.000') ## Fill 7733
# time = ('2022-06-20 08:30:00.000', '2022-06-20 11:30:00.000') ## Fill 7822
time = ('2022-07-07 01:30:00.000', '2022-07-07 03:30:00.000') ## Fill 7923


export_fbct(time, name="7923")
export_dc(time, name="7923")
